...
# 配置日志记录
logging.basicConfig(filename='yangcong.log', level=logging.DEBUG,
                    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
key_url = ''
m3u8_url = ''
vtt_url = ''
full_name_list = ['洋葱','初中-化学-人教版']
...
with sync_playwright() as p:
    ...
    def get_options():
        return page.locator('//*[contains(@class,"ant-dropdown") and contains(@class,"ant-dropdown-placement-bottomLeft") and not(contains(@class,"ant-dropdown-hidden"))]//li').all()
    ...
    for book_index in range(book_count):
        ...
        for section_index in range(article_count):
            ...
            for unit in all_unit_name_list:
                ...
                vide_types = page.locator(f'//*[{delete_quotes(unit[-1])}]/../ul/li//h4')
                for index,vide_type in enumerate(vide_types.all()):
                    ...
                    for index,article in enumerate(all_articles.all()):
                        num = f'{index+1}'.zfill(2)
                        full_name_list.append(f'{num}{all_articles_name.nth(index).inner_text()}')
                        ...
                        if key_url != '':
                            ...
                            if response.status_code == 200:
                                base64_data = base64.b64encode(response.content)  # 将内容转换为Base64编码
                                base64_string = base64_data.decode('utf-8')  # 转换为字符串形式
                        # 获取vtt_url文件
                        if vtt_url != '':
                            response = requests.get(vtt_url)
                            if response.status_code == 200:
                                file_full_path = os.path.join(current_path,f'{full_name_list[-1]}.vtt')
                                with open(file_full_path, 'wb') as f:
                                    f.write(response.content)
                        # 下载文件
                        download_tool = os.path.join(os.getcwd(),'m3u8dl','m3u8dl.exe')
                        if m3u8_url != '':
